...
function FireEvents(...)
    local buff = {...}
    for f4_local0 = 1, #buff, 1 do
        FireEvent(buff[f4_local0])
    end
    return
...
function IsNodeActive(...)
    local buff = {...}
    for f5_local0 = 1, #buff, 1 do
        if hkbIsNodeActive(buff[f5_local0]) then
            return TRUE
        end
    end
    return FALSE
...
function GetRandomIndex(max, before_index)
    local msg_array = {}
    local ret = 0
    for f9_local0 = 0, max, 1 do
        if 0 <= before_index and before_index <= max and f9_local0 == before_index then
        else
            table.insert(msg_array, f9_local0)
        end
    end
    if 0 < #msg_array then
...
function _LandReset(current_hkb_state)
    g_wallJumpCount = 0
    g_airSubAttackCount = 0
    if current_hkb_state ~= HKB_STATE_GROUND_SPECIAL_ATTACK_JUMP_READY and current_hkb_state ~= HKB_STATE_SPRINT_SPECIAL_ATTACK_JUMP_READY and current_hkb_state ~= HKB_STATE_AIR_SPECIAL_ATTACK_LANDING_JUMP_READY and current_hkb_state ~= HKB_STATE_GROUND_SPECIAL_ATTACK_JUMP_READY_NO_RESOURCE and current_hkb_state ~= HKB_STATE_SPRINT_SPECIAL_ATTACK_JUMP_READY_NO_RESOURCE and current_hkb_state ~= HKB_STATE_AIR_SPECIAL_ATTACK_LANDING_JUMP_READY_NO_RESOURCE then
        g_airSpecialAttackCount = 0
    end
...
function Control(current_hkb_state)
    ...
    if currentStyle == STYLE_TYPE_STAND then
        _LandReset(current_hkb_state)
        if (current_hkb_state == HKB_STATE_STAND_STEP_FROM_DAMAGE or current_hkb_state == HKB_STATE_STAND_STEP_FROM_DOWN) and env(1108, ACTION_ARM_USE_ITEM) > 0 and env(3036, SP_EF_REF_TAE_ACCEPT_ITEM_USE_PRIORITIZE) == TRUE then
            act(2002, SP_EF_ITEM_USE_PRIORITIZE)
        end
    ...
    local f19_local0 = {[ACTION_ARM_WALL_HANG] = FALSE, [ACTION_ARM_JUMP] = FALSE, [ACTION_ARM_SP_MOVE] = FALSE}
    ...
    local f19_local1 = function (isStandby, isMoveable, currentState)
    ...
    local f19_local2 = function (isStandby, isMoveable, currentState)
    ...
    local f19_local3 = function (isStandby, isMoveable, currentState)
    ...
    local f19_local4 = function (isStandby, isMoveable, currentState)
    ...
    local f19_local5 = function (isStandby, isMoveable, currentState)
    ...
    if g_behaviorTable[BEH_A_COVER_START][currentStyle] == TRUE then
        act(3026)
        if f19_local0[ACTION_ARM_WALL_HANG] == FALSE and g_forceCrouch == FALSE and env(3043) == TRUE and env(3036, SP_EF_REF_TAE_DISABLE_COVER_START) == FALSE and env(3008) == FALSE and _enableActionGuide(isStandby, isMoveable, currentState) == TRUE then
            act(3030, ACTION_GUIDE_COVER_START, ACTION_ARM_WALL_HANG)
            f19_local0[ACTION_ARM_WALL_HANG] = TRUE
        end
    end
    if g_behaviorTable[BEH_A_COVER_END][currentStyle] == TRUE and f19_local0[ACTION_ARM_WALL_HANG] == FALSE and f19_local3(isStandby, isMoveable, currentState) == TRUE and _enableActionGuide(isStandby, isMoveable, currentState) == TRUE then
        act(3030, ACTION_GUIDE_COVER_END, ACTION_ARM_WALL_HANG)
        f19_local0[ACTION_ARM_WALL_HANG] = TRUE
    end
    if g_behaviorTable[BEH_A_GROUND_HANG_START][currentStyle] == TRUE then
        act(3027)
        if f19_local0[ACTION_ARM_WALL_HANG] == FALSE and env(3044) == TRUE and env(3008) == FALSE and f19_local1(isStandby, isMoveable, currentState) == TRUE and _enableActionGuide(isStandby, isMoveable, currentState) == TRUE then
            act(3030, ACTION_GUIDE_HANG_START, ACTION_ARM_WALL_HANG)
            f19_local0[ACTION_ARM_WALL_HANG] = TRUE
        end
    end
    if g_behaviorTable[BEH_A_HANG_CLIMB][currentStyle] == TRUE and f19_local0[ACTION_ARM_WALL_HANG] == FALSE and env(3050) == TRUE and f19_local1(isStandby, isMoveable, currentState) == TRUE and _enableActionGuide(isStandby, isMoveable, currentState) == TRUE then
        act(3030, ACTION_GUIDE_HAMG_TO_STAND, ACTION_ARM_WALL_HANG)
        f19_local0[ACTION_ARM_WALL_HANG] = TRUE
    end
    if g_behaviorTable[BEH_A_HANG_FALL][currentStyle] == TRUE and f19_local0[ACTION_ARM_SP_MOVE] == FALSE and f19_local1(isStandby, isMoveable, currentState) == TRUE and _enableActionGuide(isStandby, isMoveable, currentState) == TRUE then
        act(3030, ACTION_GUIDE_HAMG_TO_FALL, ACTION_ARM_SP_MOVE)
        f19_local0[ACTION_ARM_SP_MOVE] = TRUE
    end
    if g_behaviorTable[BEH_A_AIR_HANG_START][currentStyle] == TRUE then
        act(3028)
        if f19_local0[ACTION_ARM_WALL_HANG] == FALSE and env(3045) ~= AIR_HANG_TYPE_DISABLE and f19_local2(isStandby, isMoveable, currentState) == TRUE and _enableActionGuide(isStandby, isMoveable, currentState) == TRUE then
            act(3030, ACTION_GUIDE_HANG_START, ACTION_ARM_WALL_HANG)
            f19_local0[ACTION_ARM_WALL_HANG] = TRUE
        end
    end
    if g_behaviorTable[BEH_R_DIVE_TO_SWIM][currentStyle] == TRUE and f19_local0[ACTION_ARM_JUMP] == FALSE and env(3039) == TRUE and f19_local5(isStandby, isMoveable, currentState) == TRUE and _enableActionGuide(isStandby, isMoveable, currentState) == TRUE then
        act(3030, ACTION_GUIDE_DIVE_TO_SWIM, ACTION_ARM_JUMP)
        f19_local0[ACTION_ARM_JUMP] = TRUE
    end
    if g_behaviorTable[BEH_A_SWIM_TO_DIVE][currentStyle] == TRUE and f19_local0[ACTION_ARM_SP_MOVE] == FALSE and env(3038) == TRUE and env(3033, ACTION_UNLOCK_TYPE_DIVE) == TRUE and env(3008) == FALSE and f19_local4(isStandby, isMoveable, currentState) == TRUE and _enableActionGuide(isStandby, isMoveable, currentState) == TRUE then
