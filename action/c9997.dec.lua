-- ============================================================================
-- 只狼：影逝二度 - 核心行为系统脚本 (c9997.dec.lua)
-- ============================================================================
--
-- 文件说明：
--   这是只狼游戏中最核心的共享行为逻辑脚本文件
--   包含所有角色（敌人、BOSS、NPC）共享的：
--   - 伤害处理系统（ExecDamage 及其责任链）
--   - AI行为系统（AI动作、攻击、防御、移动）
--   - 状态转换系统（主动/被动状态转换）
--   - 死亡处理系统（普通死亡、特殊死亡）
--   - 动画控制系统（与Havok Behavior引擎交互）
--   - 常量定义（AI状态、伤害类型、动画ID等）
--
-- 主要函数：
--   ExecDamage(transition_rank)     - 伤害处理主函数（见 2322行）
--   ExecAIAction()                  - AI行为执行
--   ExecActiveTransition()          - 主动状态转换
--   ExecPassiveTransition()         - 被动状态转换
--   ExecDeath()                     - 死亡处理
--
-- 架构特点：
--   1. 责任链模式 - 伤害处理按优先级顺序传递
--   2. 过渡等级系统 - 控制状态转换优先级
--   3. Havok集成 - 通过Fire/SetVariable与动画引擎通信
--
-- 相关文档：
...
GUARD_LEVEL_ADD = 5
guard_damage_table = {
    { GUARD_LEVEL_NONE, GUARD_LEVEL_NONE, GUARD_LEVEL_NONE, GUARD_LEVEL_NONE, GUARD_LEVEL_NONE },
    { GUARD_LEVEL_NONE, GUARD_LEVEL_SMALL, GUARD_LEVEL_SMALL, GUARD_LEVEL_SMALL, GUARD_LEVEL_SMALL },
    { GUARD_LEVEL_NONE, GUARD_LEVEL_LARGE, GUARD_LEVEL_SMALL, GUARD_LEVEL_SMALL, GUARD_LEVEL_SMALL },
    { GUARD_LEVEL_NONE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE },
    { GUARD_LEVEL_NONE, GUARD_LEVEL_EXLARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE },
    { GUARD_LEVEL_NONE, GUARD_LEVEL_SMALL, GUARD_LEVEL_SMALL, GUARD_LEVEL_SMALL, GUARD_LEVEL_LARGE },
    { GUARD_LEVEL_NONE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE },
    { GUARD_LEVEL_NONE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE },
    { GUARD_LEVEL_NONE, GUARD_LEVEL_ADD, GUARD_LEVEL_ADD, GUARD_LEVEL_ADD, GUARD_LEVEL_ADD },
    { GUARD_LEVEL_NONE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE },
    { GUARD_LEVEL_NONE, GUARD_LEVEL_EXLARGE, GUARD_LEVEL_EXLARGE, GUARD_LEVEL_EXLARGE, GUARD_LEVEL_LARGE },
    { GUARD_LEVEL_NONE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE, GUARD_LEVEL_LARGE } }
DAMAGE_LEVEL_NONE = 0
...
-- ============================================================================
-- 功能说明：
--   向Havok Behavior引擎触发一个动画事件，使角色执行对应的动画
--   同时重置旋转受影响标志
--
-- 参数：
--   event - 动画事件名称字符串（如 "W_DamageSmall", "W_GuardBreakRight" 等）
--
-- 执行操作：
--   1. 重置 IsAffectedSpin 变量为 false（清除旋转影响）
--   2. 调用 hkbFireEvent 触发Havok动画事件
--
-- 常见事件名称：
--   - "W_DamageSmall/Middle/Large" - 小/中/大伤害
--   - "W_GuardBreakRight/Left" - 右/左防御破坏
--   - "W_TrunkCollapseFront/Back" - 前/后体干崩溃
--   - "W_FireReaction" - 火焰反应
--   等等...
-- ============================================================================
function Fire(event)
...
-- ============================================================================
-- 功能说明：
--   设置Havok Behavior图中的变量值
--
-- 参数：
--   name - 变量名称字符串
--   val  - 要设置的值（可以是数字、布尔值等）
--
-- 常见变量：
--   - "BlendDamageDir" - 伤害方向混合权重
--   - "BlendDamageFire" - 火焰伤害混合权重
--   - "IndexDamageDirection" - 伤害方向索引
--   - "DamageDefaultState" - 默认伤害状态
--   - "IsAffectedSpin" - 是否受旋转影响
--   等等...
-- ============================================================================
...
--
-- 参数：
--   damaged_any       - 是否受到任何伤害的标志
--   damage_level      - 伤害等级（DAMAGE_LEVEL_*）
--   damage_type       - 伤害类型（DAMAGE_TYPE_*）
--   pre_sp_damage     - 预处理的特殊伤害类型（SP_DAMAGE_*）
--
-- 返回值：
--   TRUE  - 伤害无效，应该被忽略
--   FALSE - 伤害有效，需要处理
--
-- 判定条件（所有条件同时满足才判定为无效伤害）：
--   1. damaged_any == FALSE （没有受到任何伤害）
--   2. damage_level <= DAMAGE_LEVEL_NONE （伤害等级为0或更低）
--   3. damage_type 为 NONE 或 WEAK_FRAME （无伤害类型或弱帧类型）
--   4. pre_sp_damage == SP_DAMAGE_NONE （没有特殊伤害）
-- ============================================================================
...
--
-- 判定条件（任一满足即为空中受伤）：
--   1. SP_EFFECT_REF_AERIAL_DAMAGE 特效生效（空中伤害特效）
--   2. env(201) == FALSE （不在地面上）
-- ============================================================================
function IsDamagedAerial()
    local ret = FALSE
    if env(3036, SP_EFFECT_REF_AERIAL_DAMAGE) == TRUE or env(201) == FALSE then
        ret = TRUE
    end
    return ret
...
function LandingDecision(land_type)
    if LAND_JUMPATTACK0 <= land_type and land_type <= LAND_JUMPATTACK9 then
        if env(248) == TRUE and env(3036, SP_EFFECT_REF_LANDING_DECISION) ~= TRUE then
            return TRUE
        else
            return FALSE
        end
    elseif env(201) == TRUE and env(3036, SP_EFFECT_REF_LANDING_DECISION) ~= TRUE then
        return TRUE
    else
        return FALSE
    end
...
function FallPreventionAssist()
    if env(3036, SP_EFFECT_REF_NO_FALL_PREVENTION_ASSIST) ~= TRUE then
        act(3016, 1)
    end
...
-- ============================================================================
-- 功能说明：
--   根据当前生效的特效和行为标识，判断并返回特殊伤害类型
--   按优先级顺序检查各种特殊伤害，后面的会覆盖前面的
--
-- 返回值：
--   SP_DAMAGE_* 常量，表示特殊伤害类型：
--   - SP_DAMAGE_WIRE              : 钩锁伤害
--   - SP_DAMAGE_THROW_NEAR_REACTION : 近距离投掷反应
--   - SP_DAMAGE_THROW_FAR_REACTION  : 远距离投掷反应
--   - SP_DAMAGE_PUSH              : 推开伤害（翻滚等）
--   - SP_DAMAGE_ASSASSINATION_BLOOD : 暗杀流血
--   - SP_DAMAGE_BURST             : 爆发伤害
--   - SP_DAMAGE_ASH_BAG           : 灰袋攻击
--   - SP_DAMAGE_NONE              : 无特殊伤害（默认）
--
-- 检测顺序和条件：
--   1. 钩锁伤害：钩锁攻击特效生效 且 至少一个钩锁伤害等级启用
--   2. 近距离投掷反应：标识为投掷近反应 且 处于战斗AI状态
--   3. 远距离投掷反应：标识为投掷远反应 且 处于战斗AI状态
--   4. 推开伤害：标识为翻滚 且 启用翻滚 且 未禁用翻滚
--   5. 暗杀流血：暗杀流血特效生效
--   6. 爆发伤害：物理类型为爆发 且 满足暴击/非暴击条件
--   7. 灰袋攻击：灰袋攻击特效生效 且 未禁用灰袋反应
--
...
function GetSpDamage()
    ...
    if env(3036, SP_EFFECT_REF_NO_ALL_REACTION) == FALSE then
        if env(334, BEH_IDENTIFIER_THROW_NEAR_REACTION) == TRUE and env(3036, SP_EFFECT_REF_AI_BATTLE) == TRUE then
            ret = SP_DAMAGE_THROW_NEAR_REACTION
        end
        if env(334, BEH_IDENTIFIER_THROW_FAR_REACTION) == TRUE and env(3036, SP_EFFECT_REF_AI_BATTLE) == TRUE then
            ret = SP_DAMAGE_THROW_FAR_REACTION
        end
        if env(334, BEH_IDENTIFIER_ROLLING) == TRUE and env(3036, SP_EFFECT_REF_ENABLE_ROLLING) == TRUE and env(3036, SP_EFFECT_REF_DISABLE_ROLLING) == FALSE then
            ret = SP_DAMAGE_PUSH
        end
        if env(3036, SP_EFFECT_REF_ASSASSINATION_BLOOD) == TRUE then
            ret = SP_DAMAGE_ASSASSINATION_BLOOD
        end
        if env(284) == DAMAGE_PHYSICAL_BURST and env(3036, SP_EFFECT_REF_NO_BURST_REACTION) == FALSE and (env(3036, SP_EFFECT_REF_BURST_ENABLE) == TRUE and env(3036, SP_EFFECT_REF_BURST_ATTACK_CRETICAL) == TRUE or env(3036, SP_EFFECT_REF_BURST_ENABLE) == FALSE and env(3036, SP_EFFECT_REF_BURST_ATTACK_NONCRETICAL) == TRUE) then
            ret = SP_DAMAGE_BURST
        end
        if env(3036, SP_EFFECT_REF_ASH_BAG_ATTACK) == TRUE and env(3036, SP_EFFECT_REF_NO_ASH_BAG_REACTION) == FALSE then
            ret = SP_DAMAGE_ASH_BAG
        end
        if env(285) == DAMAGE_ELEMENT_FIRE and env(3036, SP_EFFECT_REF_FIRE_ACTION_ENABLE) == TRUE and env(3036, SP_EFFECT_REF_NO_FIRE_FIAR_REACTION) == FALSE then
            ret = SP_DAMAGE_FIRE
        end
        if (env(285) == DAMAGE_ELEMENT_LIGHTNING or env(3036, SP_EFFECT_REF_LIGHTNING_DAMAGE) == TRUE) and env(3036, SP_EFFECT_REF_LIGHTNING_DAMAGE_ENABLE) == TRUE and env(3036, SP_EFFECT_REF_NO_LIGHTNING_DAMAGE) == FALSE then
            ret = SP_DAMAGE_LIGHTNING
        end
        if env(334, BEH_IDENTIFIER_FIRE_FEAR) == TRUE and env(3036, SP_EFFECT_REF_NO_FIRE_FIAR_REACTION) == FALSE and env(3036, SP_EFFECT_REF_FIRE_ACTION_ENABLE) == TRUE then
            ret = SP_DAMAGE_FIRE_FEAR
        end
        if env(3036, SP_EFFECT_REF_FINGER_WHISTLE) == TRUE and env(3036, SP_EFFECT_REF_NO_FINGER_WHISTLE_REACTION) == FALSE then
            ret = SP_DAMAGE_FINGER_WHISTLE
        end
        if env(3041, STATUS_BURNING) == TRUE and IsExistAnime(ANIME_ID_FIRE_REACTION) == TRUE and env(3036, SP_EFFECT_REF_NO_FIRE_REACTION) == FALSE then
            ret = SP_DAMAGE_BURNING
        end
        if env(3041, STATUS_SPECIAL_POISON) == TRUE and env(3036, SP_EFFECT_REF_WOMAN) == TRUE and env(3036, SP_EFFECT_REF_WOMAN_POISON) == TRUE then
            ret = SP_DAMAGE_POISON_REACTION
        end
        if env(334, BEH_IDENTIFIER_SPIN) == TRUE and env(3036, SP_EFFECT_REF_HIDE_ACTION) == TRUE then
            ret = SP_DAMAGE_HIDE_ACTION
        end
        if env(334, BEH_IDENTIFIER_BACK_REALITY) == TRUE and env(3036, SP_EFFECT_REF_BACK_REALITY) == TRUE then
...
-- ============================================================================
-- 功能说明：
--   检查从当前状态等级能否转换到目标状态等级
--   过渡等级系统用于控制伤害处理的优先级，数字越小优先级越高
--
-- 参数：
--   cur_state_rank        - 当前状态的过渡等级（DAMAGE_TRANSITION_RANK__*）
--   destination_state_rank - 目标状态的过渡等级（DAMAGE_TRANSITION_RANK__*）
--
-- 返回值：
--   TRUE  - 允许转换
--   FALSE - 不允许转换（当前等级优先级更高）
--
-- 转换规则：
--   - RANK__0：最高优先级，总是允许转换
--   - RANK__1：仅当当前不是 RANK__0 时允许
--   - RANK__2：仅当当前不是 RANK__0、RANK__1、RANK__2 时允许
--   - RANK__3：仅当当前不是 RANK__0、RANK__1、RANK__2 时允许
--   - RANK__4：最低优先级，仅当当前不是 RANK__0~3 时允许
--
-- 简单理解：
--   只有当目标等级的优先级 >= 当前等级时才允许转换
--   高优先级状态会阻止低优先级状态的触发
-- ============================================================================
...
function IsDead()
    ...
    if hp <= 1 and env(305, SP_EFFECT_NO_DEAD) == TRUE and env(3036, SP_EFFECT_REF_NOT_TO_DEATH_ANIME) == FALSE then
        return TRUE
    end
    return FALSE
...
--
-- 计算公式：
--   当前体力 (env(1001)) / 最大体力 (env(2010))
--
-- 使用场景：
--   - 判断是否触发雷电体干崩溃 (GetStaminaRatio() <= 0)
--   - 判断是否触发爆发攻击崩溃
--   - 其他需要检查体干状态的逻辑
-- ============================================================================
function GetStaminaRatio()
    return env(1001) / env(2010)
...
--
-- 前置条件：
--   - env(3036, SP_EFFECT_REF_NOT_FACE_ATTACKER) != TRUE
--     （未启用"不面向攻击者"特效）
--
-- 执行操作：
--   调用 act(159, rad) 设置角色朝向
--
-- 使用场景：
--   - 受到伤害时转向攻击者
--   - 执行体干崩溃等反应动画前调整朝向
...
function ExecActiveTransition(is_on_cancel, style, move_type, enable_turn)
    ...
    if is_on_cancel == TRUE and env(2000) == FALSE then
        return FALSE
    end
    if env(3034) == TRUE then
    ...
    if env(3014) == TRUE then
        if env(3036, SP_EFFECT_REF_CROUCH) ~= TRUE then
            Fire("W_StandtoCrouch")
            return TRUE
        end
    elseif env(3036, SP_EFFECT_REF_CROUCH) == TRUE then
...
function ExecAIAction(is_on_cancel, style)
    ...
    if is_on_cancel == TRUE then
        local action_type_combo = env(107)
        local action_type_cancel = env(120)
        local action_type_step = env(118)
        local action_type_endure = env(123)
        local action_type_back = env(124)
        local action_type_reaction = env(125)
...
function ExecEventTrandition()
    local event_id = env(105, EVE_SLOT_ID__THROW_TRANSITION)
...
function ExecAttack(action_number)
    if action_number == 3015 and env(3036, SP_EFFECT_REF_1400_IDENTIFY) == TRUE then
        act(3014)
    end
...
function ExecTurn(is_on_cancel, style)
    ...
    local is_emergency_turn = env(330)
    ...
    elseif ai_state == AI_STATE_DEFAULT or env(3036, SP_EFFECT_REF_FLIGHT) == TRUE or env(3036, SP_EFFECT_REF_CROUCH) == TRUE then
        event = event .. "Default"
    elseif ai_state == AI_STATE_CAUTION_NO_BATTLE then
...
function ExecMove(move_type, is_on_cancel)
    ...
    elseif action_type == ACTION_TYPE_GENERAL_STANCE then
        ...
        if ai_state == AI_STATE_DEFAULT or sp_state > MOVE_STAND then
            ...
            elseif env(3036, SP_EFFECT_REF_ENABLE_NOMAL_BACK_AND_SIDE_WALK) == TRUE then
...
function ExecPassiveTransition(land_type, fall_type, transition_rank)
    ...
    if env(1112) == TRUE then
        if env(273) == ANIME_ID_THROW_DEF_ANTI_AIR and LandingDecision(land_type) ~= FALSE then
            act(144, 211)
        end
        return TRUE
    end
...
function ExecDeath(land_type)
    ...
    local event = ""
    if env(334, BEH_IDENTIFIER_SPIN) == TRUE and IsExistAnime(ANIME_ID_DEATH_HIDE) == TRUE and env(3036, SP_EFFECT_REF_HIDE_ACTION) == TRUE then
        anim_id = ANIME_ID_DEATH_HIDE
        event = "W_DeathStartHide"
    elseif env(334, BEH_IDENTIFIER_BACK_REALITY) == TRUE and env(3036, SP_EFFECT_REF_BACK_REALITY) == TRUE and (IsExistAnime(ANIME_ID_DEATH_BACK_REALITY) == TRUE or IsExistAnime(ANIME_ID_DEATH_BACK_REALITY + 1) == TRUE) then
        ...
        local damage_type = env(202)
        local damage_physical_type = env(284)
        local damage_special_type = env(285)
        local height = env(224) / 100
        if land_type ~= LAND_NOT_FALL then
            if LandingDecision(land_type) == FALSE then
                if damage_type == DAMAGE_TYPE_DEATH and env(256) == FALSE then
                    act(3014)
        ...
        elseif env(3036, SP_EFFECT_REF_AERIAL_DAMAGE) == TRUE or env(200) == TRUE then
            ...
            elseif env(3032) == DIRECTION_FB_FORWARD then
                anim_id = ANIME_ID_DEATH_AERIAL_FRONT
                event = "W_DeathAerialFront"
            elseif env(3032) == DIRECTION_FB_BACK then
                anim_id = ANIME_ID_DEATH_AERIAL_BACK
                event = "W_DeathAerialBack"
            end
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000) == TRUE then
        ...
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 1) == TRUE then
            ...
            event = "W_Event20001"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 2) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 2
            event = "W_Event20002"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 3) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 3
            event = "W_Event20003"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 4) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 4
            event = "W_Event20004"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 5) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 5
            event = "W_Event20005"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 6) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 6
            event = "W_Event20006"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 7) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 7
            event = "W_Event20007"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 8) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 8
            event = "W_Event20008"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 9) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 9
            event = "W_Event20009"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 10) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 10
            event = "W_Event20010"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 11) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 11
            event = "W_Event20011"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 12) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 12
            event = "W_Event20012"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 13) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 13
            event = "W_Event20013"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 14) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 14
            event = "W_Event20014"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 15) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 15
            event = "W_Event20015"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 16) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 16
            event = "W_Event20016"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 17) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 17
            event = "W_Event20017"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 18) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 18
            event = "W_Event20018"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 19) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 19
            event = "W_Event20019"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_EVENT20000 + 20) == TRUE then
            anim_id = ANIME_ID_ONE_SHOT_EVENT_BEGIN + 20
            event = "W_Event20020"
        elseif env(3036, SP_EFFECT_REF_TO_DEATH_UNIQUE_CASE) == TRUE then
        ...
        elseif env(3036, SP_EFFECT_REF_WOMAN) == TRUE and env(3036, SP_EFFECT_REF_WOMAN_POISON) == TRUE and env(3041, STATUS_SPECIAL_POISON) == TRUE or env(3036, SP_EFFECT_REF_WOMAN) == TRUE and env(3036, SP_EFFECT_REF_SPECIAL_POISON) == TRUE then
            anim_id = ANIME_ID_DEATH_SPECIAL_POISON
            event = "W_DeathStartSpecialPoison"
        elseif env(334, BEH_IDENTIFIER_SPIN) == TRUE and IsExistAnime(ANIME_ID_DEATH_HIDE) == TRUE and env(3036, SP_EFFECT_REF_HIDE_ACTION) == TRUE then
            anim_id = ANIME_ID_DEATH_HIDE
            event = "W_DeathStartHide"
        elseif env(334, BEH_IDENTIFIER_BACK_REALITY) == TRUE and env(3036, SP_EFFECT_REF_BACK_REALITY) == TRUE and (IsExistAnime(ANIME_ID_DEATH_BACK_REALITY) == TRUE or IsExistAnime(ANIME_ID_DEATH_BACK_REALITY + 1) == TRUE) then
            local index = GetRandomIndex(ANIME_ID_DEATH_BACK_REALITY, 0, 1, 1, 0, "IndexDeathStartBackReality", true)
        ...
        elseif damage_physical_type ~= DAMAGE_PHYSICAL_NO_ABILITY or damage_special_type ~= DAMAGE_ELEMENT_NONE then
            ...
            if damage_special_type == DAMAGE_ELEMENT_FIRE and env(3036, SP_EFFECT_REF_FIRE_ACTION_ENABLE) == TRUE then
                special_death_anim_id = ANIME_ID_DEATH_FIRE
                special_death_event = "W_DeathStartFire"
            elseif (damage_special_type == DAMAGE_ELEMENT_LIGHTNING or env(3036, SP_EFFECT_REF_LIGHTNING_DAMAGE) == TRUE) and env(3036, SP_EFFECT_REF_LIGHTNING_DAMAGE_ENABLE) == TRUE then
...
function ExecSpDeath()
    if env(1000) > 0 and env(3036, SP_EFFECT_REF_NOT_RESURRECTION) == FALSE then
        if env(3036, SP_EFFECT_REF_RESURRECTION_IDLE_UPWARD) == TRUE then
            if IsExistAnime(ANIME_ID_RESURRECTION_IDLE_UPWARD) == TRUE then
                Fire("W_ResurrectionIdleUpward")
                return TRUE
            end
        elseif env(3036, SP_EFFECT_REF_RESURRECTION_IDLE_DOWNWARD) == TRUE and IsExistAnime(ANIME_ID_RESURRECTION_IDLE_DOWNWARD) == TRUE then
            Fire("W_ResurrectionIdleDownward")
            return TRUE
        end
    else
        if env(3036, SP_EFFECT_REF_EXPLOSION) == TRUE then
...
function ExecTransToDeathIdle(death_type, is_tmp_death)
    ...
    local anim_id = INVALID
    local index = INVALID
    if env(3036, SP_EFFECT_REF_TO_DEATH_IDLE_UNIQUE_CASE) == TRUE then
...
-- ============================================================================
-- 功能说明：
--   这是只狼伤害系统的核心函数，按严格的优先级顺序处理所有伤害反应
--   使用责任链模式，依次尝试各种伤害处理器，直到某个处理器成功处理
--
-- 参数：
--   transition_rank - 当前过渡等级（DAMAGE_TRANSITION_RANK__*）
--                     用于判断当前状态是否允许被新伤害打断
--
-- 返回值：
--   TRUE  - 成功处理伤害
...
-- 【初始化阶段】
--   1. 读取伤害参数：等级、类型、方向等
--   2. 获取特殊伤害类型 (GetSpDamage)
--   3. 判断是否空中受伤 (IsDamagedAerial)
--   4. 执行旋转反应 (ExecSpinReaction)
--   5. 验证伤害有效性 (IsInvalidDamage) - 无效则直接返回FALSE
--   6. 重置伤害混合变量
--
-- 【责任链处理阶段 - 按优先级顺序】
--
--   优先级1: ExecDebuffReaction (RANK__3/4)
--   └─ 处理：燃烧反应、毒素反应等持续性负面状态
--
--   优先级2: ExecGuardBlock (无rank检查)
--   └─ 处理：防御破坏、完美弹反、普通格挡
--
--   优先级3: ExecDamageLargeBlow (RANK__0)
--   └─ 处理：大型击退、上挑、爆炸等强力伤害
--
--   优先级4: ExecDamageBreakSp (RANK__1)
--   └─ 处理：雷电体干崩溃、爆发攻击崩溃等特殊破坏
--
--   优先级5: ExecSpReactionLarge (RANK__3)
--   └─ 处理：火焰伤害、雷电伤害、恐火反应等大型特殊反应
--
--   优先级6: ExecDamageBreak (RANK__2)
--   └─ 处理：体干崩溃（前/后/空中）
--
--   优先级7: ExecDamageBlow (无rank检查)
--   └─ 处理：小型击退伤害
--
--   优先级8: ExecBound (无rank检查)
--   └─ 处理：攻击被弹反时的反作用力
--
--   优先级9: ExecDamageDefault (RANK__4)
--   └─ 处理：小/中/大等常规伤害等级
--
--   优先级10: ExecSpReaction (RANK__4)
--   └─ 处理：投掷物反应、暗杀流血、推开等小型特殊反应
--
--   兜底处理: ExecNoSyncAddDamage
--   └─ 处理：所有处理器都失败时的非同步附加伤害
--
-- ============================================================================
-- 过渡等级拒绝机制：
-- ============================================================================
--   - 某些处理器会检查过渡等级，如果当前等级优先级更高则拒绝转换
--   - 被拒绝时返回 REJECTED_BY__DAMAGE_TRANSITION_RANK
--   - rejected_by_transition_rank 标志会被设置，影响兜底处理的逻辑
--
-- ============================================================================
-- 返回值说明：
-- ============================================================================
--   处理器返回 TRUE：
--     → ExecDamage 立即返回TRUE，中断责任链
--
--   处理器返回 FALSE：
--     → 继续尝试下一个处理器
--
--   处理器返回 REJECTED_BY__DAMAGE_TRANSITION_RANK：
--     → 设置拒绝标志，但继续尝试下一个处理器
--
--   处理器返回 TRNAS_TO_ADD (仅GuardBlock)：
--     → ExecDamage 返回FALSE，表示转换到附加伤害状态
--
-- ============================================================================
-- 相关文档：
--   参见 sekiro_damage_flow.md 了解完整的伤害计算流程图
-- ============================================================================
function ExecDamage(transition_rank)
    -- ========== 【初始化阶段】读取伤害参数 ==========
    local damage_level = env(236)        -- 伤害等级 (DAMAGE_LEVEL_*)
    local damage_type = env(202)         -- 伤害类型 (DAMAGE_TYPE_*)
    local damaged_any = env(256)         -- 是否受到任何伤害
    local damage_direction = env(222)    -- 伤害方向
    local pre_sp_damage = GetSpDamage()  -- 获取特殊伤害类型
    local damaged_aerial = IsDamagedAerial()  -- 检查是否空中受伤
    local rejected_by_transition_rank = FALSE  -- 过渡等级拒绝标志
    local transition_result = INVALID    -- 处理器返回结果
...
-- 防御格挡伤害处理函数
-- ============================================================================
-- 职责：处理角色受到防御相关伤害时的反应动作和动画播放
-- 参数：
--   damage_level        伤害等级（DAMAGE_LEVEL_SMALL/MIDDLE/LARGE）
--   damage_type         伤害类型（防御破坏/普通防御等）
--   damage_direction    伤害方向（前/后/左/右）
-- 返回：
--   TRUE                成功处理了防御伤害
--   FALSE               无法处理此伤害
-- ============================================================================
function ExecGuardBlock(damage_level, damage_type, damage_direction)
    -- 获取防御方向状态（玩家的完美招架方向和角色当前防御方向）
    local just_guard_direction = env(3057)  -- 玩家完美招架方向
    local guard_direction = env(3056)       -- 角色当前防御方向

    -- 处理防御破坏伤害
    if damage_type == DAMAGE_TYPE_GUARD_BREAK then
        -- 防御被破坏时的处理流程
        Replanning()                            -- 重新规划AI行为
        FaceAttacker(0)                         -- 面向攻击者
        act(141, DAMAGE_FLAG_GUARD_BREAK)      -- 设置防御破坏标记
        act(144, 211)                           -- 执行防御破坏动作命令
        -- 根据破坏方向播放对应的防御破坏被击动画
        if just_guard_direction == GUARD_DIR_RIGHT or guard_direction == GUARD_DIR_RIGHT then
            Fire("W_GuardBreakRight")           -- 从右侧防御破坏的被击动画
        elseif just_guard_direction == GUARD_DIR_LEFT or guard_direction == GUARD_DIR_LEFT then
            Fire("W_GuardBreakLeft")            -- 从左侧防御破坏的被击动画
        end
        return TRUE
    -- 处理普通防御伤害（防御成功接收伤害或踩破体力时）
    elseif damage_type == DAMAGE_TYPE_GUARD or damage_type == DAMAGE_TYPE_GUARD_ATTAKER_STAM_ZERO then
        -- 获取防御状态信息
        local is_justguard = env(3036, SP_EFFECT_REF_ENEMY_JUST_GUARD)        -- 是否处于完美招架状态
        local is_specialguard = env(3036, SP_EFFECT_REF_SPECIAL_GUARD)         -- 是否有特殊防御效果1
        local is_specialguard2 = env(3036, SP_EFFECT_REF_SPECIAL_GUARD2)       -- 是否有特殊防御效果2
        local guard_level = env(258)                                           -- 防御等级

        -- 使用损伤等级和防御等级查表，确定这次防御伤害的对应反应
        -- guard_damage_table 是一个二维查表，[伤害等级][防御等级] -> 防御反应类型
        local guard_damage_action = guard_damage_table[damage_level + 1][guard_level + 1]

        -- 处理附加伤害（Special Attack，能够增加防御度）的防御伤害
        if guard_damage_action == GUARD_LEVEL_ADD or guard_damage_action ~= GUARD_LEVEL_NONE and env(3036, SP_EFFECT_REF_GUARD_TO_ADD_DAMAGE) == TRUE then
            act(141, DAMAGE_FLAG_GUARD_BREAK)                                  -- 标记为防御破坏伤害
            -- 检查是否存在附加伤害混合动画（Super Armor Blend）
            if IsExistAnime(ANIME_ID_ADD_BLEND_FRONT) == TRUE then
                -- 根据伤害方向确定播放的动画
                local offsetanim = GetDamageDirectionOffset(damage_direction)  -- 获取方向偏移值
                local str = "W_SABlend_Add_Front"                              -- 默认前向动画
                -- 根据伤害方向选择不同的动画
                if damage_direction == DIRECTION_FORWARD then
                    -- 前向保持默认
                elseif damage_direction == DIRECTION_LEFT then
                    str = "W_SABlend_Add_Left"                                 -- 左方向动画
                elseif damage_direction == DIRECTION_RIGHT then
                    str = "W_SABlend_Add_Right"                                -- 右方向动画
                elseif damage_direction == DIRECTION_BACK then
                    str = "W_SABlend_Add_Back"                                 -- 后方向动画
                end
                -- 检查是否存在对应方向的动画，如果存在则播放，否则回退到前向动画
                if IsExistAnime(ANIME_ID_ADD_BLEND_FRONT + offsetanim) == TRUE then
                    SetVariable("BlendSA", 0.5)                               -- 设置超甲混合强度为0.5
                    Fire(str)                                                 -- 播放对应方向的超甲混合动画
                else
                    SetVariable("BlendSA", 0.5)
                    Fire("W_SABlend_Add_Front")                               -- 回退到前向动画
                end
                return TRNAS_TO_ADD                                            -- 返回特殊状态"转变为附加伤害模式"
            end
        -- 处理小伤害防御（轻微防御伤害）
        elseif guard_damage_action == GUARD_LEVEL_SMALL then
            act(141, DAMAGE_FLAG_GUARD_SMALL)                                  -- 标记为小伤害防御
            -- 从右侧来的防御伤害
            if just_guard_direction == GUARD_DIR_RIGHT or guard_direction == GUARD_DIR_RIGHT then
                -- 优先级1：完美招架+特殊防御效果1 -> 播放特殊完美招架动画
                if is_justguard == TRUE and is_specialguard == TRUE and IsExistAnime(ANIME_ID_SPECIALJUSTGUARD_RIGHT_TO_LEFT) == TRUE then
                    act(144, 211)
                    Replanning()
                    Fire("W_JustGuardDamageSpecial_RighttoLeft")              -- 特殊防御1的完美招架受击动画
                    return TRUE
                -- 优先级2：完美招架+特殊防御效果2
                elseif is_justguard == TRUE and is_specialguard2 == TRUE and IsExistAnime(ANIME_ID_SPECIALJUSTGUARD2_RIGHT_TO_LEFT) == TRUE then
                    act(144, 211)
                    Replanning()
                    Fire("W_JustGuardDamageSpecial2_RighttoLeft")             -- 特殊防御2的完美招架受击动画
                    return TRUE
                -- 优先级3：仅完美招架
                elseif is_justguard == TRUE and IsExistAnime(ANIME_ID_JUSTGUARD_RIGHT_TO_LEFT) == TRUE then
                    act(144, 211)
                    Replanning()
                    Fire("W_JustGuardDamage_RighttoLeft")                     -- 普通完美招架受击动画
                    return TRUE
                -- 优先级4：特殊防御效果1（无完美招架）
                elseif is_specialguard == TRUE and IsExistAnime(ANIME_ID_SPECIALGUARD_RIGHT_TO_LEFT) == TRUE then
                    act(144, 211)
                    Replanning()
                    Fire("W_GuardDamageSpecial_RighttoLeft")                  -- 特殊防御1受击动画
                    return TRUE
                -- 优先级5：特殊防御效果2（无完美招架）
                elseif is_specialguard2 == TRUE and IsExistAnime(ANIME_ID_SPECIALGUARD2_RIGHT_TO_LEFT) == TRUE then
                    act(144, 211)
                    Replanning()
                    Fire("W_GuardDamageSpecial2_RighttoLeft")                 -- 特殊防御2受击动画
                    return TRUE
                -- 优先级6：普通防御小伤害
                elseif IsExistAnime(ANIME_ID_SMALL_GUARD_RIGHT_TO_LEFT) == TRUE then
                    act(144, 211)
                    Replanning()
                    Fire("W_GuardDamageSmall_RighttoLeft")                    -- 普通防御小伤害受击动画
                    return TRUE
                -- 无合适动画，返回失败
                else
                    return FALSE
        ...
        elseif guard_damage_action == GUARD_LEVEL_LARGE then
            act(141, DAMAGE_FLAG_GUARD_LARGE)                                  -- 标记为大伤害防御
            -- 从右侧来的防御伤害
            if just_guard_direction == GUARD_DIR_RIGHT or guard_direction == GUARD_DIR_RIGHT then
                -- 优先级1：完美招架+特殊防御效果1
                if is_justguard == TRUE and is_specialguard == TRUE and IsExistAnime(ANIME_ID_SPECIALJUSTGUARD_RIGHT_TO_LEFT) == TRUE then
            ...
            elseif just_guard_direction == GUARD_DIR_LEFT or guard_direction == GUARD_DIR_LEFT then
                ...
                elseif damage_level == DAMAGE_LEVEL_SMALL_BLOW and IsExistAnime(ANIME_ID_KICK_GUARD_LEFT_TO_RIGHT) == TRUE then
                    act(144, 211)
                    Replanning()
                    Fire("W_GuardKick_LefttoRight")                            -- 踢击防御的被击动画（左到右）
                    return TRUE
                -- 优先级7：普通防御大伤害
                elseif IsExistAnime(ANIME_ID_LARGE_GUARD_LEFT_TO_RIGHT) == TRUE then
                    act(144, 211)
                    Replanning()
                    Fire("W_GuardDamageLarge_LefttoRight")                      -- 普通防御大伤害受击动画（左到右）
                    return TRUE
                -- 无合适动画，返回失败
                else
                    return FALSE
...
-- 特殊伤害破坏处理函数（体干崩溃）
-- ============================================================================
-- 职责：处理造成体干崩溃（Trunk Collapse）的特殊伤害
--       当角色体力耗尽时，被雷电或爆炸伤害击中会进入崩溃状态
-- 参数：
--   damage_level        伤害等级
--   damage_type         伤害类型
--   transition_rank     状态转换优先级
--   pre_sp_damage       预设的特殊伤害类型（雷电/爆炸等）
-- 返回：
--   TRUE                成功处理了特殊伤害破坏
--   FALSE               无法处理此伤害
--   REJECTED_BY__DAMAGE_TRANSITION_RANK - 被优先级拒绝
-- 注意：这个函数处理的是"体干崩溃"状态，不是普通的伤害破坏
-- ============================================================================
function ExecDamageBreakSp(damage_level, damage_type, transition_rank, pre_sp_damage)
    -- 前置检查：排除某些伤害等级（击退/超级爆炸不能导致体干崩溃）
    if env(349) == TRUE or damage_level == DAMAGE_LEVEL_BLOW or damage_level == DAMAGE_LEVEL_EX_BLAST then
        return FALSE
    else
        -- 获取物理伤害类型和特殊伤害类型
        local damage_physical_type = env(284)                                  -- 物理伤害类型（爆炸等）
        local damage_special_type = env(285)                                   -- 特殊伤害类型
        ...
        if pre_sp_damage == SP_DAMAGE_LIGHTNING and GetStaminaRatio() <= 0 then
            ...
            -- 重新规划AI行为并播放雷电体干崩溃开始动画
            Replanning()                                                        -- 重新规划AI
            act(144, 211)                                                      -- 执行动作命令
            Fire("W_TrunkCollapseLightningStart")                              -- 播放雷电导致的体干崩溃开始动画
            return TRUE
        end
        ...
        --   3. 有爆炸攻击暴击效果
        --   4. 存在对应的体干崩溃爆炸动画
        if damage_physical_type == DAMAGE_PHYSICAL_BURST and GetStaminaRatio() <= 0 and
           env(3036, SP_EFFECT_REF_BURST_ATTACK_CRETICAL) == TRUE and
           (IsExistAnime(ANIME_ID_TRUNK_COLLAPSE_BURST) or
            IsExistAnime(ANIME_ID_TRUNK_COLLAPSE_BURST + 1) or
            IsExistAnime(ANIME_ID_TRUNK_COLLAPSE_BURST + 2)) then
            ...
            -- 随机选择3个爆炸体干崩溃动画中的一个（0-2编号）
            local index = GetRandomIndex(ANIME_ID_TRUNK_COLLAPSE_BURST, 0, 2, 1, 0, "IndexTrunkCollapseBurst", true)
            SetVariable("IndexTrunkCollapseBurst", index)                       -- 保存选中的动画索引

            -- 执行体干崩溃相关命令
            act(141, DAMAGE_FLAG_GUARD_BREAK)                                  -- 标记为防御破坏伤害
            Replanning()                                                        -- 重新规划AI行为
            act(3009)                                                           -- 执行特殊命令（可能是禁用某些反应）
            act(144, 211)                                                      -- 执行动作命令
            FaceAttacker(0)                                                     -- 面向攻击者
            Fire("W_TrunkCollapseBurst")                                        -- 播放爆炸体干崩溃动画
            -- 注意：这里没有 return TRUE，继续往下执行
        end
    end
    -- 没有特殊伤害被处理，返回失败（交给其他处理函数处理）
    return FALSE
...
-- 伤害破坏处理函数
-- ============================================================================
-- 职责：处理破坏类伤害（Damage Break）
--       当角色受到破坏伤害时，进入"体干崩溃"状态
--       这是一个强制状态转换，会立即改变角色行为
-- 参数：
--   damage_type         伤害类型（必须是 DAMAGE_TYPE_DAMAGE_BREAK）
--   damage_level        伤害等级
--   damaged_aerial      是否处于空中
--   transition_rank     状态转换优先级
-- 返回：
--   TRUE                成功处理了破坏伤害
--   FALSE               无法处理此伤害
--   REJECTED_BY__DAMAGE_TRANSITION_RANK - 被优先级或其他条件拒绝
-- ============================================================================
function ExecDamageBreak(damage_type, damage_level, damaged_aerial, transition_rank)
    ...
    -- 检查2：是否处于反应安全时间内（某些特殊状态下无法转换）
    if env(3036, SP_EFFECT_REF_REACTION_SAFE_TIME) == TRUE then
        return REJECTED_BY__DAMAGE_TRANSITION_RANK
    end

    -- 检查3：排除某些伤害等级（击退/超级爆炸不能导致破坏）
    if env(349) == TRUE or damage_level == DAMAGE_LEVEL_BLOW or damage_level == DAMAGE_LEVEL_EX_BLAST then
        return FALSE
    end

    -- 检查4：检查状态转换优先级（优先级2，比体干崩溃低）
    if IsEnabledTransitionRank(transition_rank, DAMAGE_TRANSITION_RANK__2) == FALSE then
        return REJECTED_BY__DAMAGE_TRANSITION_RANK
    end

    -- 通过所有检查，开始执行破坏伤害处理
    act(3009)                                                                   -- 执行特殊命令（禁用某些反应）
    Replanning()                                                               -- 重新规划AI行为
    act(141, DAMAGE_FLAG_GUARD_BREAK)                                          -- 标记为防御破坏伤害

    -- 获取伤害方向和伤害物理类型
    local damage_direction = env(3032)                                         -- 伤害来源方向
    local damage_physical_type = env(284)                                      -- 物理伤害类型
    ...
    -- 优先级1：爆炸伤害破坏
    if damage_physical_type == DAMAGE_PHYSICAL_BURST and
       (IsExistAnime(ANIME_ID_TRUNK_COLLAPSE_BURST) or
        IsExistAnime(ANIME_ID_TRUNK_COLLAPSE_BURST + 1) or
        IsExistAnime(ANIME_ID_TRUNK_COLLAPSE_BURST + 2)) then
        -- 随机选择3个爆炸体干崩溃动画中的一个
    ...
    elseif damaged_aerial == TRUE and IsExistAnime(ANIME_ID_TRUNK_COLLAPSE_AERIAL) == TRUE then
        -- 根据伤害方向和特殊效果决定是否转身
        if damage_direction == DIRECTION_FB_BACK and env(3036, SP_EFFECT_REF_BREAK_FACE_FRONT) == FALSE then
            FaceAttacker(180)                                                   -- 转向180度（背向攻击者）
        else
            FaceAttacker(0)                                                     -- 面向攻击者
        end
        act(144, 211)                                                          -- 执行动作命令
        Fire("W_TrunkCollapseAerial")                                          -- 播放空中体干崩溃动画

    -- 优先级3：背向伤害破坏（无特殊效果或允许背向）
    elseif damage_direction == DIRECTION_FB_BACK and env(3036, SP_EFFECT_REF_BREAK_FACE_FRONT) == FALSE then
        FaceAttacker(180)                                                      -- 转向180度
        act(144, 211)                                                          -- 执行动作命令
        -- 优先选择背向动画，如不存在则回退到前向
        if IsExistAnime(ANIME_ID_TRUNK_COLLAPSE_BACK) == TRUE then
            Fire("W_TrunkCollapseBack")                                        -- 播放背向体干崩溃动画
        else
            Fire("W_TrunkCollapseFront")                                       -- 回退到前向动画
        end
    ...
    -- 优先级4：默认前向伤害破坏
        ...
        FaceAttacker(0)                                                         -- 面向攻击者
        act(144, 211)                                                          -- 执行动作命令
        Fire("W_TrunkCollapseFront")                                           -- 播放前向体干崩溃动画
    end
...
function ExecDamageBlow(damage_level, damage_direction)
    if damage_level == DAMAGE_LEVEL_SMALL_BLOW or damage_level == DAMAGE_LEVEL_BREATH then
        local damage_direction = env(3032)
...
-- 大型击退伤害处理函数
-- ============================================================================
-- 职责：处理造成大型击退（Large Blow）的伤害
--       这是优先级很高的伤害类型（DAMAGE_TRANSITION_RANK__0）
--       会使角色受到较大的击退动作
-- 参数：
--   damage_level        伤害等级（BLOW/UPPER/EX_BLAST）
--   damaged_aerial      是否处于空中
--   transition_rank     状态转换优先级
-- 返回：
--   TRUE                成功处理了大型击退伤害
...
--
-- 触发条件：
--   1. 伤害等级必须是击退类型（BLOW/UPPER/EX_BLAST）
--   2. 必须存在对应的击退动画
--   3. 状态转换优先级必须允许（最高优先级 RANK__0）
-- ============================================================================
function ExecDamageLargeBlow(damage_level, damaged_aerial, transition_rank)
    -- 分支1：处理空中大型击退伤害
    if damaged_aerial == TRUE then
        -- 检查：伤害等级符合 + 空中动画存在
        if (damage_level == DAMAGE_LEVEL_BLOW or damage_level == DAMAGE_LEVEL_UPPER or damage_level == DAMAGE_LEVEL_EX_BLAST) and
           IsExistAnime(ANIME_ID_DAMAGE_LARGE_BLOW_AERIAL) == TRUE then

            -- 检查状态转换优先级（最高优先级 RANK__0）
            if IsEnabledTransitionRank(transition_rank, DAMAGE_TRANSITION_RANK__0) == FALSE then
                return REJECTED_BY__DAMAGE_TRANSITION_RANK                      -- 被优先级拒绝
            end

            -- 执行空中大型击退处理
            FaceAttacker(0)                                                     -- 面向攻击者
            SetVariable("DamageDefaultState", 1)                               -- 设置伤害默认状态标记
            ExecSyncAddDamage(damage_level)                                    -- 执行同步附加伤害处理
            Replanning()                                                        -- 重新规划AI行为
            act(144, 211)                                                      -- 执行动作命令
            Fire("W_DamageAerialBlow")                                         -- 播放空中大型击退动画
            return TRUE
    ...
    -- 分支2：处理地面大型击退伤害
    elseif (damage_level == DAMAGE_LEVEL_BLOW or damage_level == DAMAGE_LEVEL_EX_BLAST) and
           IsExistAnime(ANIME_ID_DAMAGE_LARGE_BLOW) == TRUE then
        ...
        Replanning()                                                            -- 重新规划AI行为
        act(144, 211)                                                          -- 执行动作命令
        Fire("W_DamageLargeBlow")                                              -- 播放地面大型击退动画
        return TRUE
    end
...
-- 弹跳效果处理函数
-- ============================================================================
-- 职责：处理防御后产生弹跳的伤害（Bound）
--       玩家完美招架（Parry）后会产生特定的弹跳效果
--       根据弹跳类型和方向选择相应的受击动画
-- 参数：
--   damage_type         伤害类型（包含弹跳信息）
--   damaged_aerial      是否处于空中
-- 返回：
--   TRUE                成功处理了弹跳效果
--   FALSE               无法处理此弹跳
--
-- 弹跳类型分类：
--   1. 完美招架弹跳 (DAMAGE_TYPE_GUARDED_BOUND_ENEMY1/2/3)
--   2. 体力耗尽完美招架弹跳 (DAMAGE_TYPE_GUARDED_STAM_ZERO)
--   3. 普通防御弹跳 (DAMAGE_TYPE_GUARDED_NO_BOUND_ENEMY1/2/3)
--   4. 附加防御弹跳 (BOUND_ADD01-04，特殊招架效果)
-- ============================================================================
function ExecBound(damage_type, damaged_aerial)
    -- 检查伤害类型是否存在
    if damage_type ~= DAMAGE_TYPE_NONE then
        -- 处理完美招架弹跳（包括攻击弹跳和体力耗尽弹跳）
...
-- ============================================================================
-- 职责：处理体力耗尽时完美招架产生的弹跳
--       当角色体力为0时被完美招架，会进入特殊的"体力耗尽弹跳"状态
-- 参数：
--   just_guard_bound_type   完美招架时的弹跳方向（LEFT/RIGHT/NONE/ADD01-04）
--   damaged_aerial          是否处于空中
-- 返回：
--   TRUE                    成功处理了体力耗尽弹跳
--   FALSE                   无法处理（缺少动画或其他原因）
-- ============================================================================
function ExecAttackBoundBreak(just_guard_bound_type, damaged_aerial)
    ...
    if damaged_aerial == TRUE then
        ...
        if IsExistAnime(ANIME_ID_EMPTY_STAMINA_ATTACK_BOUND_AERIAL_RIGHT) == TRUE then
            act(144, 211)                                                      -- 执行动作命令
            Replanning()                                                        -- 重新规划AI
            Fire("W_AttackBoundEmptyStaminaEnemyAerial_Right")                 -- 播放右侧空中体力耗尽弹跳动画
            return TRUE
        -- 尝试播放左侧空中体力耗尽弹跳动画
        elseif IsExistAnime(ANIME_ID_EMPTY_STAMINA_ATTACK_BOUND_AERIAL_LEFT) == TRUE then
            act(144, 211)
            Replanning()
            Fire("W_AttackBoundEmptyStaminaEnemyAerial_Left")                  -- 播放左侧空中体力耗尽弹跳动画
            return TRUE
        end
        return FALSE                                                            -- 无合适的空中动画
    ...
    -- 分支2：地面的体力耗尽弹跳
        ...
        if just_guard_bound_type == BOUND_RIGHT or just_guard_bound_type == BOUND_NON then
            if IsExistAnime(ANIME_ID_EMPTY_STAMINA_ATTACK_BOUND_RIGHT) == TRUE then
                act(144, 211)
                Replanning()
                Fire("W_AttackBoundEmptyStaminaEnemy_Right")                   -- 播放右侧地面体力耗尽弹跳动画
                return TRUE
            end
        -- 左侧弹跳
        elseif just_guard_bound_type == BOUND_LEFT then
            if IsExistAnime(ANIME_ID_EMPTY_STAMINA_ATTACK_BOUND_LEFT) == TRUE then
                act(144, 211)
                Replanning()
                Fire("W_AttackBoundEmptyStaminaEnemy_Left")                    -- 播放左侧地面体力耗尽弹跳动画
                return TRUE
            end
        -- 附加防御类型的弹跳（ADD01-ADD04）
...
-- 完美招架弹跳处理函数
-- ============================================================================
-- 职责：处理玩家完美招架（Parry）产生的弹跳效果
--       根据攻击来源敌人和弹跳方向选择相应的受击动画
-- 参数：
--   damage_type             伤害类型（DAMAGE_TYPE_GUARDED_BOUND_ENEMY1/2/3）
--   just_guard_bound_type   完美招架弹跳方向（LEFT/RIGHT/NONE）
-- 返回：
--   TRUE                    成功处理了完美招架弹跳
--   FALSE                   无法处理（无匹配动画）
--
-- 攻击来源分类：
--   ENEMY1 - 普通攻击弹跳
--   ENEMY2 - 特殊攻击弹跳1（SPECIAL1）
--   ENEMY3 - 特殊攻击弹跳2（SPECIAL2）
-- ============================================================================
...
-- 普通防御弹跳处理函数
-- ============================================================================
-- 职责：处理普通防御（非完美招架）产生的弹跳效果
--       当攻击被普通防御格挡时，会产生无弹跳的受击动画
--       这种情况下不产生明显的弹跳效果，仅改变受击动画
-- 参数：
--   damage_type         伤害类型（DAMAGE_TYPE_GUARDED_NO_BOUND_ENEMY1/2/3）
--   guard_bound_type    普通防御弹跳方向（LEFT/RIGHT/NONE）
-- 返回：
--   TRUE                成功处理了普通防御弹跳
--   FALSE               无法处理（无匹配动画）
--
-- 攻击来源分类：
--   ENEMY1 - 普通攻击受击
--   ENEMY2 - 特殊攻击受击1（SPECIAL1）
--   ENEMY3 - 特殊攻击受击2（SPECIAL2）
-- ============================================================================
function ExecAttackBound(damage_type, guard_bound_type)
    ...
    if guard_bound_type == BOUND_RIGHT then
        ...
        if damage_type == DAMAGE_TYPE_GUARDED_NO_BOUND_ENEMY1 and IsExistAnime(ANIME_ID_ATTACK_NO_BOUND_RIGHT) == TRUE then
            Replanning()                                                        -- 重新规划AI
            Fire("W_AttackNoBoundEnemy1_Right")                                -- 播放右侧普通受击动画
            return TRUE
        -- 特殊攻击受击1
        elseif damage_type == DAMAGE_TYPE_GUARDED_NO_BOUND_ENEMY2 and IsExistAnime(ANIME_ID_ATTACK_NO_BOUND_RIGHT_SPECIAL1) == TRUE then
            Replanning()
            Fire("W_AttackNoBoundEnemy2_Right")                                -- 播放右侧特殊受击1动画
            return TRUE
        -- 特殊攻击受击2
        elseif damage_type == DAMAGE_TYPE_GUARDED_NO_BOUND_ENEMY3 and IsExistAnime(ANIME_ID_ATTACK_NO_BOUND_RIGHT_SPECIAL2) == TRUE then
            Replanning()
            Fire("W_AttackNoBoundEnemy3_Right")                                -- 播放右侧特殊受击2动画
            return TRUE
        end
...
-- 附加防御完美招架弹跳处理函数
-- ============================================================================
-- 职责：处理特殊防御效果（附加防御 ADD01-ADD04）下的完美招架弹跳
--       这些特殊防御状态下，完美招架会触发特殊的混合受击动画
-- 参数：
--   just_guard_bound_type   完美招架弹跳类型（BOUND_ADD01/ADD02/ADD03/ADD04）
-- 返回：
--   TRUE                    成功处理或无法处理（特殊回退机制）
--   FALSE                   无法处理此弹跳
-- ============================================================================
function ExecAttackAddJustBound(just_guard_bound_type)
    -- 检查是否存在"不应用附加"动画（如果不存在则返回TRUE）
    if IsExistAnime(ANIME_ID_NO_ADD) == FALSE then
        return TRUE                                                             -- 无附加动画，提前返回
    end

    -- 循环检查4种附加防御类型（BOUND_ADD01-ADD04）
    for f79_local0 = 0, 3, 1 do
        ...
        if just_guard_bound_type == BOUND_ADD01 + f79_local0 then
            ...
            if IsExistAnime(ANIME_ID_PARRY_BOUND_ADD01 + f79_local0) == FALSE then
                return TRUE                                                     -- 动画不存在，提前返回
            end
            -- 附加类型编号（1-4）
            str = f79_local0 + 1
            SetVariable("AttackJustGuardBoundBlend", 1)                         -- 设置混合强度
            Fire("W_AttackJustGuardBound_Add0" .. str)                          -- 播放对应的附加防御完美招架弹跳动画
            return TRUE
        end
    end
    -- 未找到匹配的附加防御类型
...
-- 附加防御普通弹跳处理函数
-- ============================================================================
-- 职责：处理特殊防御效果（附加防御 ADD01-ADD04）下的普通防御弹跳
--       这些特殊防御状态下，普通防御会触发特殊的混合受击动画
-- 参数：
--   guard_bound_type        普通防御弹跳类型（BOUND_ADD01/ADD02/ADD03/ADD04）
-- 返回：
--   TRUE                    成功处理或无法处理（特殊回退机制）
--   FALSE                   无法处理此弹跳
-- ============================================================================
function ExecAttackAddBound(guard_bound_type)
    ...
    for f80_local0 = 0, 3, 1 do
        ...
        if guard_bound_type == BOUND_ADD01 + f80_local0 then
            ...
            -- 附加类型编号（1-4）
            str = f80_local0 + 1
            SetVariable("AttackGuardBoundBlend", 1)                            -- 设置混合强度
            Fire("W_AttackGuardBound_Add0" .. str)                             -- 播放对应的附加防御普通弹跳动画
            return TRUE
        end
    end
    -- 未找到匹配的附加防御类型
...
function ExecNoSyncAddDamage(damage_level, damaged_any, damage_direction, damaged_aerial, rejected_by_transition_rank)
    if damage_level == DAMAGE_LEVEL_MINIMUM and damaged_any == TRUE or damaged_aerial == TRUE and damaged_any == TRUE or rejected_by_transition_rank == TRUE then
        if env(336) == TRUE then
            return TRUE
        end
        if IsExistAnime(ANIME_ID_NO_ADD) == FALSE then
            return TRUE
        end
        local damage_part = env(1120)
...
-- ============================================================================
-- 功能说明：
--   检查并处理旋转攻击（Spin Attack）造成的特殊反应
--   如果受到旋转攻击，会设置旋转受影响标志并调整角色朝向
--
-- 返回值：
--   TRUE  - 成功触发旋转反应
--   FALSE - 未触发旋转反应
--
-- 触发条件（同时满足）：
--   1. env(334, BEH_IDENTIFIER_SPIN) == TRUE （行为标识为旋转攻击）
--   2. env(3036, SP_EFFECT_REF_NO_SPIN) == FALSE （未禁用旋转效果）
--
-- 执行操作：
--   - 设置 IsAffectedSpin 变量为 true
--   - act(159, 180) 转向180度（背对攻击者）
-- ============================================================================
function ExecSpinReaction()
    if env(334, BEH_IDENTIFIER_SPIN) == TRUE and env(3036, SP_EFFECT_REF_NO_SPIN) == FALSE then
...
-- ============================================================================
-- 功能说明：
--   处理燃烧和毒素等持续性负面状态造成的伤害反应
--   这是伤害处理链中优先级较高的处理器之一
--
-- 参数：
--   damage_level    - 伤害等级（未直接使用，但保持参数一致性）
--   pre_sp_damage   - 特殊伤害类型（SP_DAMAGE_*）
--   transition_rank - 当前过渡等级，用于检查是否允许状态转换
--
-- 返回值：
--   TRUE  - 成功处理了Debuff反应，中断后续伤害处理
--   FALSE - 未处理Debuff反应，继续下一个处理器
--   REJECTED_BY__DAMAGE_TRANSITION_RANK - 被过渡等级拒绝，记录拒绝标志
--
-- 处理的Debuff类型：
--   1. SP_DAMAGE_BURNING (燃烧反应)
--      - 需要过渡等级 >= RANK__3
--      - 触发 "W_FireReaction" 动画
--
--   2. SP_DAMAGE_POISON_REACTION (毒素反应)
--      - 需要过渡等级 >= RANK__4（最低优先级）
--      - 触发 "W_SpecialPoisonReaction" 动画
--
-- 前置条件：
--   - env(349) == FALSE （不在无敌状态）
--
-- 执行操作：
--   - Replanning() - 重新规划AI行为
--   - act(144, 211) - 执行引擎命令（可能是打断当前动作）
--   - Fire() - 触发对应的反应动画事件
-- ============================================================================
function ExecDebuffReaction(damage_level, pre_sp_damage, transition_rank)
    if env(349) == FALSE then
...
function ExecSpReactionLarge(damage_level, pre_sp_damage, transition_rank)
    if env(349) == FALSE then
        ...
        if pre_sp_damage == SP_DAMAGE_LIGHTNING and env(3036, SP_EFFECT_REF_LIGHTNING_DAMAGE_ENABLE) == TRUE then
            if IsEnabledTransitionRank(transition_rank, DAMAGE_TRANSITION_RANK__3) == FALSE then
                return REJECTED_BY__DAMAGE_TRANSITION_RANK
            end
            Replanning()
        ...
        if env(3036, SP_EFFECT_REF_1500_UG_DAMAGE) == TRUE and damage_level ~= DAMAGE_LEVEL_NONE and damage_level ~= DAMAGE_LEVEL_MINIMUM then
            if IsEnabledTransitionRank(transition_rank, DAMAGE_TRANSITION_RANK__3) == FALSE then
                return REJECTED_BY__DAMAGE_TRANSITION_RANK
            end
            Replanning()
    ...
    elseif pre_sp_damage == SP_DAMAGE_WIRE then
        ...
        Replanning()
        act(144, 211)
        if env(3036, SP_EFFECT_REF_ENABLE_WIRE_DAMAGE0) == TRUE then
            Fire("W_DamageWire0")
        elseif env(3036, SP_EFFECT_REF_ENABLE_WIRE_DAMAGE1) == TRUE then
            Fire("W_DamageWire1")
        elseif env(3036, SP_EFFECT_REF_ENABLE_WIRE_DAMAGE2) == TRUE then
            Fire("W_DamageWire2")
        elseif env(3036, SP_EFFECT_REF_ENABLE_WIRE_DAMAGE3) == TRUE then
            Fire("W_DamageWire3")
        else
            Fire("W_DamageWire4")
        end
...
function ExecDamageDefault(damage_level, damage_direction, damaged_aerial, transition_rank)
    if damaged_aerial == TRUE then
        if damage_level == DAMAGE_LEVEL_MIDDLE or damage_level == DAMAGE_LEVEL_LARGE or damage_level == DAMAGE_LEVEL_PUSH or damage_level == DAMAGE_LEVEL_FLING or damage_level == DAMAGE_LEVEL_SMALL_BLOW then
            if env(3032) == DIRECTION_FB_BACK and IsExistAnime(ANIME_ID_DAMAGE_AERIAL_BACK) == TRUE then
                if IsEnabledTransitionRank(transition_rank, DAMAGE_TRANSITION_RANK__4) == FALSE then
                    return REJECTED_BY__DAMAGE_TRANSITION_RANK
                end
...
-- 同步附加伤害处理函数
-- ============================================================================
-- 职责：处理与主伤害同步播放的附加混合动画
--       例如被击退时的火焰或方向混合效果
-- 参数：
--   damage_level        伤害等级（SMALL/MIDDLE/LARGE）
-- 返回：
--   TRUE                成功处理了附加伤害
--   FALSE               无法处理或不需要处理
--
...
--   2. 优先处理火焰伤害的特殊混合
--   3. 根据伤害等级和方向选择混合动画
--   4. 支持方向回退（找不到该方向则尝试其他方向）
-- ============================================================================
function ExecSyncAddDamage(damage_level)
    -- 检查1：是否处于某种保护/免疫状态（env(336)）
    if env(336) == TRUE then
        return FALSE                                                            -- 处于保护状态，不应用附加伤害
    end

    -- 检查2：是否存在混合伤害动画集合
    if IsExistAnime(ANIME_ID_NO_ADD) == FALSE then
        return FALSE                                                            -- 无混合动画集合，无法处理
    end

    -- 特殊处理：火焰伤害的混合效果
    if env(285) == DAMAGE_ELEMENT_FIRE and IsExistAnime(ANIME_ID_BLEND_DAMAGE_FIRE) == TRUE then
        SetVariable("BlendDamageFire", 1)                                      -- 设置火焰混合强度
        Fire("W_DamageFireBlend_Add")                                          -- 播放火焰混合伤害动画
        return TRUE
    end

    -- 根据伤害等级准备候选动画ID表（支持降级）
    -- 例如 LARGE 等级会依次尝试 LARGE->MIDDLE->SMALL
    local buff = {[1] = -1}                                                     -- 初始化候选表
    ...
    -- 尝试找到合适的方向混合动画
    if buff[1] >= 0 then
        -- 获取角色的真实方向（Front/Back/Left/Right）
        local dir = GetRealDir()                                               -- 0=Front, 1=Back, 2=Left, 3=Right
        local anim_id = -1
        ...
        for f87_local0 = 1, #buff, 1 do
            ...
            if IsExistAnime(buff[f87_local0] + dir) == TRUE then
                anim_id = buff[f87_local0]                                     -- 找到可用动画
                break                                                           -- 立即停止遍历
            end
        end
        ...
        if anim_id >= 0 then
            -- 根据选中的动画等级确定事件名称
            local event = "W_DirDamageSmall"                                    -- 默认小伤害
            if anim_id == ANIME_ID_BLEND_DAMAGE_MIDDLE_FRONT then
                event = "W_DirDamageMiddle"                                    -- 中伤害
            elseif anim_id == ANIME_ID_BLEND_DAMAGE_LARGE_FRONT then
                event = "W_DirDamageLarge"                                     -- 大伤害
            end

            -- 方向后缀表（对应0-3的方向）
            local f87_local1 = {"Front_Add", "Back_Add", "Left_Add", "Right_Add"}
            SetVariable("BlendDamageDir", 1)                                   -- 设置方向混合强度
            -- 组合事件名称并播放（例如 W_DirDamageLarge + Front_Add）
...
-- 获取相对方向函数
-- ============================================================================
-- 职责：根据角色位置方向和攻击来源方向，计算相对于角色的受击方向
--       用于选择正确的方向混合动画（Front/Back/Left/Right/Up/Down）
-- 返回：
--   相对方向常量（FRONT_ADD/BACK_ADD/LEFT_ADD/RIGHT_ADD/FRONTUP_ADD/FRONTDOWN_ADD）
--
-- 工作原理：
--   1. 获取角色当前面向方向 (pos_dir)
--   2. 获取攻击来源方向 (atk_dir)
--   3. 相对方向 = 攻击方向 相对于 角色面向方向
--   4. 考虑攻击的垂直方向（前/上/下）
-- ============================================================================
function GetRealDir()
    -- 获取角色当前面向方向和攻击来源相对方向
    local pos_dir = env(222)                                                    -- 角色面向方向（0-3或类似编码）
    local atk_dir = env(1119)                                                  -- 攻击来源方向（ATTACK_DIR_*）
    ...
    elseif atk_dir == ATTACK_DIR_UP then
        ...
        elseif pos_dir == DIRECTION_RIGHT then
            real_dir = RIGHT_ADD                                               -- 侧向
        elseif pos_dir == DIRECTION_BACK then
            real_dir = BACK_ADD                                                -- 背向
        elseif pos_dir == DIRECTION_LEFT then
            real_dir = LEFT_ADD                                                -- 侧向
        end
    ...
    -- 攻击来自左侧方向
    -- ========================================================================
    -- 注：这里使用相对于角色面向的左侧，需要考虑角色方向旋转
    elseif atk_dir == ATTACK_DIR_LEFT then
        if pos_dir == DIRECTION_FORWARD then
            -- 角色面向前方，攻击来自左侧 -> 相对右侧受击（角色顺时针旋转）
            real_dir = RIGHT_ADD
        elseif pos_dir == DIRECTION_RIGHT then
            -- 角色面向右方，攻击来自左侧 -> 背向受击
            real_dir = BACK_ADD
        elseif pos_dir == DIRECTION_BACK then
            -- 角色面向后方，攻击来自左侧 -> 相对左侧受击
            real_dir = LEFT_ADD
        elseif pos_dir == DIRECTION_LEFT then
            -- 角色面向左方，攻击来自左侧 -> 正面受击
            real_dir = FRONT_ADD
    ...
    elseif atk_dir == ATTACK_DIR_RIGHT then
        if pos_dir == DIRECTION_FORWARD then
            -- 角色面向前方，攻击来自右侧 -> 相对左侧受击（角色逆时针旋转）
            real_dir = LEFT_ADD
        elseif pos_dir == DIRECTION_LEFT then
            -- 角色面向左方，攻击来自右侧 -> 背向受击
            real_dir = BACK_ADD
        elseif pos_dir == DIRECTION_BACK then
            -- 角色面向后方，攻击来自右侧 -> 相对右侧受击
            real_dir = RIGHT_ADD
        elseif pos_dir == DIRECTION_RIGHT then
            -- 角色面向右方，攻击来自右侧 -> 正面受击
            real_dir = FRONT_ADD
...
function ExecFallStart(fall_type)
    ...
    local anim_id = INVALID
    local event = ""
    if env(3036, SP_EFFECT_REF_1130_FALL_START_DOWNWARD) == TRUE then
        event = "W_FallingDownward"
        anim_id = ANIME_ID_FALLING_DOWNWARD
    elseif env(3036, SP_EFFECT_REF_1130_FALL_START_UPWARD) == TRUE then
...
function ExecLand(land_type)
    ...
    elseif land_type >= LAND_JUMPATTACK0 and land_type <= LAND_JUMPATTACK9 then
        Replanning()
        if (land_type == LAND_JUMPATTACK0 or land_type == LAND_JUMPATTACK1) and env(3036, SP_EFFECT_REF_1470_IDENTIFY) == TRUE then
            act(3014)
        end
...
function ExecLadderNextAction(hand, is_idle)
    ...
    if action_type == LADDER_REQUEST_UP then
        ...
        if env(294, CONFLICT_DIST_CHR, UPSIDE, PLAYER) == TRUE then
            local stamina = env(1001)
        ...
        if env(291, CONFLICT_DIST_CHR, 0) == TRUE then
            if is_idle == TRUE and env(294, 0, UPSIDE, ENEMY) == TRUE and env(303) == TRUE then
    ...
    elseif action_type == LADDER_REQUEST_DOWN then
        ...
        if env(294, CONFLICT_DIST_CHR, DOWNSIDE, PLAYER) == TRUE then
            local stamina = env(1001)
            if stamina < LADDER_ATTACK_STAMINA_THRESHOLD then
                return FALSE
            end
        ...
        if env(292, CONFLICT_DIST_COAST, 0) == FALSE then
            Fire("W_LadderCoastStart")
            return TRUE
        end
        if is_idle == TRUE and env(292, CONFLICT_DIST_CHR, 0) == TRUE then
...
function ExecLadderCoastNextAction(hand)
    ...
    if env(292, CONFLICT_DIST_CHR, 0) == FALSE then
...
function SetThrowFlag(state)
    ...
    elseif state == STATE_DEATH then
        ...
        local ai_throw_target = env(3062)
        state_throwing_side = THROWABLE_STATE_ATK_ENE_BASE
        if env(3036, SP_EFFECT_REF_HEADWATER) == TRUE then
            state_thrown_side = THROWABLE_STATE_DEF_ENE_HEADWATER
        elseif env(3036, SP_EFFECT_REF_UNDERWATER) == TRUE or env(3036, SP_EFFECT_REF_BOTTOMWATER) == TRUE then
    ...
    if env(3036, SP_EFFECT_REF_DISABLE_THROWN) == TRUE then
        state_thrown_side = THROWABLE_STATE_DEF_COMMON_IMPOSSIBLE
    end
    act(160, state_throwing_side)
    act(161, state_thrown_side)
...
function JumpAttackCommonFunction(land_type, action_number)
    act(3021, 1)
    FallPreventionAssist()
    if env(3036, SP_EFFECT_REF_JUMP_BEFORE) == TRUE then
...
function ThrowCommonFunction()
    if env(206) == TRUE then
        ...
        if env(3036, SP_EFFECT_REF_WIRE_ATTACK) == TRUE and env(3036, SP_EFFECT_REF_5020_WIRE_DAMAGE1) == TRUE then
            Replanning()
            act(144, 211)
            Fire("W_DamageWire1")
            return TRUE
...
function ThrowAtkCommonFunction()
    ...
    if env(274) == TRUE and IsExistAnime(throw_id) == TRUE then
        act(136, THROW_STATE_NONE)
        Fire("W_ThrowKill" .. throw_id)
        return TRUE
    end
    if env(275) == TRUE and IsExistAnime(throw_id) == TRUE then
        act(136, THROW_STATE_NONE)
...
function ThrowDefCommonFunction()
    ...
    if env(3036, SP_EFFECT_REF_1130_NEAR_CLIFF) == TRUE then
        local wall_angle = GetVariable("FallProtectWallAngle")
        local fall_angle = 75
        if math.abs(wall_angle) <= fall_angle and env(3030) == TRUE then
            if env(3036, SP_EFFECT_REF_1130_PERMISSION_FALL_FRONT) == TRUE then
                act(136, THROW_STATE_NONE)
                Fire("W_Event20000")
                return TRUE
            elseif env(3036, SP_EFFECT_REF_1130_PERMISSION_FALL_BACK) == TRUE then
                act(136, THROW_STATE_NONE)
                Fire("W_Event20004")
                return TRUE
            end
...
function LadderMoveCommonFunction(hand)
    ...
    if env(339, 1) == TRUE and ExecLadderNextAction(hand, FALSE) == TRUE then
        return TRUE
    end
    return FALSE
...
function LadderCoastCommonFunction(hand)
    ...
    if env(339, 1) == TRUE and ExecLadderCoastNextAction(hand) == TRUE then
        return TRUE
    end
    return FALSE
...
function Init_onUpdate()
    act(136, THROW_STATE_NONE)
    SetThrowFlag(STATE_DEATH)
    local event_id = env(105, EVE_SLOT_ID__DEATH_REPRODUCTION)
    ...
    if env(202) == DAMAGE_TYPE_DEATHIDLE or env(1000) <= 0 then
        SetVariable("IndexDeathIdleVariation", 0)
        Fire("W_DeathIdleDefault")
        return TRUE
    end
    if env(254) == TRUE then
        if env(3036, SP_EFFECT_REF_1510_SWITCH_GENERATE) == TRUE then
            Fire("W_Event21000")
            return TRUE
        else
            Fire("W_Generate")
...
function IdleBattle_onUpdate()
    SetThrowFlag(STATE_NORMAL)
    act(3029, TALK_REF_BATTLE_STATE)
...
function IdleUniqueDefault402_onUpdate()
    ...
    FallPreventionAssist()
    CallActionState(402)
    if not GetVariable("IsRequestedBgmChange") and env(3036, SP_EFFECT_REF_DELAY_BGM_REQUEST) == FALSE then
...
function DamageLightningLoop_onUpdate()
    ...
    if env(339, 1) == TRUE then
        Replanning()
        if env(3036, SP_EFFECT_REF_LIGHTNING_LOOP_LIFE) == TRUE then
            local index = GetRandomIndex(ANIME_ID_SPECIAL_DAMAGE_LIGHTNING_LOOP, 0, 2, 1, 0, "IndexDamageLightningLoop", true)
            SetVariable("IndexDamageLightningLoop", index)
            Fire("W_DamageLightningLoop")
        else
...
function DamageAerialFront_onUpdate()
    SetThrowFlag(STATE_FALL)
    if env(3036, SP_EFFECT_REF_AERIAL_DAMAGE_TO_DIRECT_LOOP) ~= TRUE then
...
function TrunkCollapseFront_onUpdate()
    SetThrowFlag(STATE_NORMAL)
    FallPreventionAssist()
    if env(334, BEH_IDENTIFIER_KICK) == TRUE then
        FaceAttacker(0)
...
function AttackBoundEnemy1_Right_onUpdate()
    SetThrowFlag(STATE_NORMAL)
    FallPreventionAssist()
    if env(3036, SP_EFFECT_REF_1550_SHIELD_TYPE) == TRUE then
...
function ResurrectionIdleUpward_onUpdate()
    SetThrowFlag(STATE_DEATH)
    if env(3036, SP_EFFECT_REF_RESURRECTION_IDLE_UPWARD) == FALSE and IsExistAnime(ANIME_ID_RESURRECTION_START_UPWARD) == TRUE then
...
function ResurrectionIdleDownward_onUpdate()
    SetThrowFlag(STATE_DEATH)
    if env(3036, SP_EFFECT_REF_RESURRECTION_IDLE_DOWNWARD) == FALSE and IsExistAnime(ANIME_ID_RESURRECTION_START_DOWNWARD) == TRUE then
...
function DeathStartBlast_onUpdate()
    SetThrowFlag(STATE_DEATH)
    FallPreventionAssist()
    if env(200) == TRUE and IsExistAnime(ANIME_ID_FALLING_UPWARD + 1) == TRUE then
...
function DeathAerialFront_onUpdate()
    ...
    if env(200) == TRUE or env(339, 1) == TRUE then
...
function ThrowDef12900_onUpdate()
    if env(3036, SP_EFFECT_REF_5010_EVENT_TRANSITION) == TRUE then
        if env(339, 1) == TRUE then
            Replanning()
            Fire("W_Event21003")
            return TRUE
        end
    elseif env(3036, SP_EFFECT_REF_SAME_THRWO_DEF) == TRUE then
        if ThrowDefCommonFunction() == TRUE then
            return TRUE
        end
    elseif env(3036, SP_EFFECT_REF_SAME_THRWO_DEF_DEATH) == TRUE then
        FallPreventionAssist()
        if ExecSpDeath() == TRUE then
            return TRUE
        end
...
function ThrowDef12950_onUpdate()
    if env(3036, SP_EFFECT_REF_SAME_THRWO_DEF) == TRUE then
        if ThrowDefCommonFunction() == TRUE then
            return TRUE
        end
    elseif env(3036, SP_EFFECT_REF_SAME_THRWO_DEF_DEATH) == TRUE then
...
function StandtoCrouch_onActivate()
    act(2002, SP_EFFECT_CROUCH)
    act(2002, 200100)
    if env(3036, SP_EFFECT_REF_WEAPON_0) == TRUE then
        g_WeponType = 0
    elseif env(3036, SP_EFFECT_REF_WEAPON_1) == TRUE then
        g_WeponType = 1
    elseif env(3036, SP_EFFECT_REF_WEAPON_2) == TRUE then
        g_WeponType = 2
    elseif env(3036, SP_EFFECT_REF_WEAPON_3) == TRUE then
        g_WeponType = 3
    elseif env(3036, SP_EFFECT_REF_WEAPON_4) == TRUE then
        g_WeponType = 4
    end
    act(2002, 200030)
...
function Update()
    ...
    if env(3058) == TRUE and GetVariable("LipSyncBlend00") <= 0 then
