...
output_file = r'D:\Sekiro\Sekiro_AI\param\param\AtkParam_c7100_弦一郎.csv'
...
def main():
    ...
    for encoding in encodings:
        try:
            with open(input_file, 'r', encoding=encoding) as f:
                ...
                rows_7100 = []
                for row in reader:
                    if row['ID'].startswith('7100'):
                        rows_7100.append(row)

                data = (header, rows_7100)
                print(f"成功使用 {encoding} 编码读取文件")
                print(f"找到 {len(rows_7100)} 行弦一郎（7100）的攻击参数")
                break
    ...
    with open(output_file, 'w', encoding='utf-8-sig', newline='') as f:
        ...
        for row in rows_7100:
            filtered_row = {col: row.get(col, '') for col in available_cols}
            writer.writerow(filtered_row)

    print(f"\n数据已保存到: {output_file}")
    print(f"共提取 {len(available_cols)} 列，{len(rows_7100)} 行数据")

    # 显示前几行作为预览
    if rows_7100:
        print("\n=== 前5行数据预览 ===")
        for i, row in enumerate(rows_7100[:5], 1):
